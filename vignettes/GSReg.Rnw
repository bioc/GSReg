...
%\VignetteKeywords{Gene Set Analysis, Analysis of Variance}
...
behind {\it DIRAC} is to generate a template for the pair-wise comparisons of gene expressions of a pathway within a phnotype. DIRAC caculates a measure of the variability of the ordering within the phenotype, i.e. the expected distance ofa sample from the phenotype and the template of the phenotype. In mathematical terms, if we denote denote two i.i.d. samples from the same phenotypes by $X$ and $X'$ and $D$ Kendall-$\tau$-distance on the specific pathways, then the GSVReg statistic is $E(D(X,X'))$. It identifies significantly dysregulated pathways by estimating p-values from a permutation test. 
...
However, the permutation test in DIRAC is computationally intensive and reaching low p-values may be impractical since they require a huge number of permutations. Low p-values are required for multiple hypothesis correction. A similar measure of variability of the orderings of gene sets was proposed in \cite{AfsariDiss}. This method approximates the p-value theoretically,  without a permutation test. This method is based on Kendall-$\tau$ distance \cite{kendalltaudistance} and the theory of U-Statistics, thus we call this method Gene Set Variation-Regulation (or in short GSVReg). Specifically, Kendall-$\tau$ distance between two expression profiles counts the number of disagreeing pairwise comparisons between two profiles. The GsVReg measures the variability of the gene expression of pathway genes from a phenotype by calculating the expectation of Kendall-$\tau$ distance between two random samples from the phenotype. GSVReg then identifies if the variability is significantly different across two phenotypes. To approximate this p-value GSVReg applies a U-Statistic Theory approach.
...
\begin{enumerate}
  ...
  \item Identifying the dysregulated pathways with {\it GSVReg} measure of variability. The significance is approximated through applying U-statistics theory. This is very time efficient and consistent with both {\it DIRAC} and applying permutation test on GSVReg. 
...
Here, we demonstrate how to use the GSReg package to compute DIRAC and GSVReg statistics.
...
system.time({VarAnKendallV = GSReg.GeneSets.VReg(geneexpres=exprsdata,
...
\subsection{Comparison of DIRAC and GSVReg}

We ran the following code to compare statistics from DIRAC and from GSVReg.
\vspace{5 mm}
<<eval=TRUE,echo=TRUE>>=
Nperm = 10;
...
{
  VarAnPerm[[i]] = GSReg.GeneSets.VReg(geneexpres=exprsdata, pathways=diracpathways, 
...
The figure represents that the theoretical p-value and p-value calculated from permutation test  in GSVReg are very similar and we can use the theoretical p-value as a surrogate for p-value. Here is the histogram.
<<fig=TRUE>>=
...
\end{figure}
Figure \ref{fig::theoempi} shows the result of comparing p-value GSVReg computing from 1000 permutation test and approximation using U-statistics theory (offline generated).
...
plot(x=DIRACAn$pvalues,y=pvalustat,xlab ="DIRAC",
        ylab="GSVReg",main=sprintf("P-value Comparison corr=%2.2g",cor(x=DIRACAn$pvalues,y=pvalustat)))
...
\caption{Comparing p-values GSVReg versus DIRAC. The correlation is 0.88.}\label{fig::DIRAC_vs_G}
\end{figure}


DIRAC and GSVReg have been shown mathematically similar. The main advantages of the GSVReg is efficiency in calculation as well as easier interpretation. Figure \ref{fig::DIRAC_vs_G} a graphical example of such comparison. One can see that the p-values generated by DIRAC and GSVReg have high correlation, i.e. 0.88. Note that GSVReg is much faster than DIRAC. For example, in this case, we ran the computations on a Lenovo Thinkpad with Core(TM) i7-3720QM Intel CPU @2.6 GHz. For a thousand permutation, the DIRAC analysis took 207.47 seconds while the latter only took 0.3 seconds. Note that for multiple hypothesis adjustment, a thousand permutations may not be satisfactory and we require hundreds of thousand or a million permutation which may not be feasible.  
\vspace{5 mm}


Note that it is possible that some of the genes in a pathway are not represented in the expression data or are too short (e.g. less than 5 genes). Both \Robject{GSReg.GeneSets.VReg} and \Robject{GSReg.GeneSets.DIRAC} may ignore such pathways through parameter \Robject{minGeneNum}. Please see the manual for more details.   
\vspace{5 mm}
If we the user wants to compare the results of DIRAC and GSVReg, they can run the following code for plot DIRAC diagram of significantly perturbed pathways: 
\vspace{5 mm}
<<eval=FALSE>>=
...
Now, if we do the analysis using GSVReg, we have:
\vspace{5 mm}
...
plot(x=eta1,y=eta2,xlim=c(0,max(eta1,eta2)),ylim=c(0,max(eta1,eta2)),xlab="normal",ylab="disease",
     main="(b) GSVReg: Dysregulated pathways")
lines(x=c(0,max(eta1,eta2)),y=c(0,max(eta1,eta2)))
@

\vspace{5 mm}
Although there is discrepancy in identified dysregulated pathways (p-value<0.05), the general trend found in \cite{DIRAC2010} holds still true. The trend is that usually the dysregulated pathways have higher variability measure in more dangerous phenotypes. The figures reveal that both DIRAC and GSVReg have this property. DIRAC found \Sexpr{length(significantPathwaysDIRAC)} dysregulatd pathways and GSVReg discovered \Sexpr{length(significantPathwaysGSV)} pathways, \Sexpr{length(intersect(significantPathwaysGSV,significantPathwaysDIRAC))} pathways showed up in both analysis, and \Sexpr{length(union(significantPathwaysGSV,significantPathwaysDIRAC))} pathways were discovered totally. 
\vspace{5 mm}
<<>>=
